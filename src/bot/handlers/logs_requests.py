...
async def callback_admin_logs(
    ...
    query = select(AdminLogModel).order_by(desc(AdminLogModel.created_at)).limit(20)
        ...
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="✅ Ko'rib chiqilganlar", callback_data="requests:processed"
                ),
            ],
            [
...
async def callback_pending_requests(
    ...
    keyboard.append([InlineKeyboardButton(text=buttons.BTN_BACK, callback_data="admin:requests")])
